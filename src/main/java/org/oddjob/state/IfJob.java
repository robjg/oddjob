...
/**
 * @author Rob Gordon
 * @oddjob.description This job implements an if/then/else logic based on job state. This job can
 ...
 * @oddjob.example If a file exists.
 * <p>
 * {@oddjob.xml.resource org/oddjob/state/IfFileExistsExample.xml}
 * @oddjob.example An example showing lots of if's. All these if's go to COMPLETE state
 * when run.
 * <p>
 * {@oddjob.xml.resource org/oddjob/state/if.xml}
 * @oddjob.example Asynchronous evaluation. Only when the first job moves beyond it's ACTIVE
        ...
        class AsyncAction implements Runnable {
            ...
            public void run() {
                ...
                asyncSupport = new AsyncExecutionSupport(
                        ...
                        t -> {
                            logger().error("Async Exception: ", t);
