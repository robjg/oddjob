...
public class JobStateHandlerTest extends OjTestCase {
    ...
    private static class RecordingStateListener implements StateListener {
    ...
    public void testManyListeners() throws Exception {
        ...
        Thread t = new Thread(() -> setState(test, JobState.COMPLETE));
    ...
    public void testListenerConcurrentModification() {
        ...
        test.waitToWhen(new IsAnyState(), () -> {
    ...
    public void testThatAttemptsToChangeState() {
        ...
        StateListener listener = event -> test.waitToWhen(new IsAnyState(), () -> {
            test.setState(JobState.COMPLETE);
            test.fireEvent();
        });
    ...
    public void testListenerNotificationOrder() throws InterruptedException {
        ...
        final Exchanger<Void> exchanger = new Exchanger<>();

        test.addStateListener(event -> {
            try {
                if (event.getState() == JobState.COMPLETE) {
        ...
        final List<State> events = new ArrayList<>();

        StateListener listener = event -> events.add(event.getState());

        new Thread(() -> test.waitToWhen(new IsAnyState(), () -> {
            test.setState(JobState.COMPLETE);
            test.fireEvent();
        })).start();
    ...
    public void testSleep() throws InterruptedException {
        ...
        Thread t1 = new Thread(() -> test.waitToWhen(new IsAnyState(), () -> {
            latch.countDown();
            try {
                test.sleep(0);
            } catch (InterruptedException e) {
